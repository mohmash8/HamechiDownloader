...
export async function upsertUser(tgId: number, username?: string) {
  const q = `insert into users (tg_id, username) values ($1,$2)
             on conflict (tg_id) do update set username=excluded.username returning id`;
  const { rows } = await pool.query(q, [tgId, username||null]);
...
export async function touchCooldown(tgId: number) {
...
export async function incUsage(tgId: number) {
