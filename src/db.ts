...
export async function insertJob(userId: number, url: string, provider: string) {
  const { rows } = await pool.query(
    `insert into jobs (user_id, url, provider) values ($1,$2,$3) returning id`,
    [userId, url, provider]
  );
  return rows[0].id as number;
}

export async function updateJob(id: number, patch: Record<string, any>) {
  const keys = Object.keys(patch);
  if (!keys.length) return;
  const set = keys.map((k, i) => `${k}=$${i+1}`).join(',');
  const vals = keys.map(k => patch[k]);
  await pool.query(`update jobs set ${set} where id=${id}`, vals);
}
